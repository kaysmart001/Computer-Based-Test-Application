    ...
    <body>
        ...
        <div id="wrapper">
            ...
            if (isset($_GET["delusers"])) {
                ...
                $sql_del = "DELETE FROM assessments_users WHERE assessment_code = '$ass_id'";
                $sql_del = mysqli_query($dbc, $sql_del);
                header("Location: add_test.php?ass=$ass_id"); die();
            }
            ...
            if (isset($_POST["import_test"])) {
                ...
                $test_id = test_input($params['test_id']);
                $ass_id = test_input($params['assess_id']);
                $counter = 1;
                    ...
                    if(is_uploaded_file($_FILES['filename']['tmp_name'])) {
                        ...
                        $rows = [];
                         ...
                         // print_r($prep_query); die();
                        

                        while (($data = fgetcsv($handle, 1500, ",")) !== FALSE) {

                            $q_id = $test_id."-".$counter++;
                ...
                if($test_id !="0"){
                   ...
                   // echo $sql_update;
                    ...
                    if ($sql_result) {
                       ...
                       $sql_insert = "INSERT INTO tests (id, TName, NOQ, be_default, prof_or_user, random, time, test_type, show_answers, show_mark, active, start_message, end_message, show_rank, test_mark, assessments_id ) 
                                   VALUES( '$id','$TName', '$NOQ','$be_default', '$prof_or_user', '$RandomQ', '$time:00', '$test_type', '$ShowAns',  '$ShowMark', '$active', '$start_message', '$end_message',  '$ShowRank', '$test_mark', '$assessment_id')";
                    $sql_result = mysqli_query($dbc, $sql_insert);
                    if ($sql_result) {
                ...
                $userid = explode("\n", $_POST['userid']);
                    ...
                    for($idx = 0; $idx < count($userid); $idx++){
                        $useridd = test_input($userid[$idx]);
                              $sql_select = "SELECT * FROM assessments_users where username = '$useridd' AND assessment_code = '$ass_id' AND session_id = $csession_id"; 
                            $countusas =  countSQL($sql_select, $dbc);
                    ...
                    if( $uploaded ){
                        ...
                        $uploadstat = "<p style='color :red'>Upload Unsuccessful! The following students could not be uploaded:<br/>";
            ...
            if (isset($_POST["edit_user_time"]) && isset($_GET['edittime'])) {

                $user_id = $_GET['edittime'];
                $time = $_POST["time"];

                 $sql_unfinish = "UPDATE `user_test` 
                                SET `finish` = '0' 
                                WHERE `user_id` = '$user_id'";
                  $sql_unfinish = mysqli_query($dbc, $sql_unfinish);


                 $sql_update = "UPDATE `user_assessments` 
                                SET `time_length` = '$time' 
                                WHERE `ass_user_id` = '$user_id'
                                AND `ass_id` = '$ass_id'";

                $sql_result = mysqli_query($dbc, $sql_update);
                if ($sql_result) {
                    $msg = "User Time Changed Successfully";     
                } else {
                    $msg_err = mysqli_error($dbc);
                }
                //get test id
            ...
            if (isset($_GET["reset_user"])) {
                ...
                $user_id = $_GET["reset_user"];
                       
                $sql_update = "DELETE FROM `user_test` WHERE `user_id` = 1";
                $sql_result = mysqli_query($dbc, $sql_update);
                if ($sql_result) {
                    if($test_status == 1){
                        $msg = "Test Activated Successfully";
            ...
            if (isset($_GET["reset_all_time"])) {


                $ass_id = $_GET["reset_all_time"];
                $test_id = $_GET["tsid"];
                 ...
                 $sql_unfinish = "UPDATE `user_test` 
                  ...
                  //print_r($sql_unfinish); die();
                  $sql_unfinish = mysqli_query($dbc, $sql_unfinish);



                 $sql_update = "UPDATE `user_assessments` 
                                SET `time_length` = '00:00' 
                                WHERE `ass_id` = '$ass_id'";

                $sql_result = mysqli_query($dbc, $sql_update);
                if ($sql_result) {
                    $msg = "All Users Time Changed Successfully";     
                } else {
                    $msg_err = mysqli_error($dbc);
                }
                header("Location: add_test.php?ass=$ass_id"); die();
                                    ...
                                    <div class="modal-body">
                                        <div class="col-sm-12">                                       
                                            ...
                                            <div class="form-group input-group">
                                                ...
                                                <input type="text" name="batch" class="form-control"  value="1" required>
                                            </div>
                                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr <?php if($row['be_default'] == 1){ echo 'class="success"'; }?> >
                                            <td> <?php echo $n; ?> </td>
                                            ...
                                            <td data-id=" <?php echo $row['id']; ?>">
                                                    ...
                                                    <div class="btn-group">
                                                            ...
                                                            <ul class="dropdown-menu pull-right" role="menu">
                                                                    ...
                                                                    <li class="divider"></li>
                                                                    <li><a href="?ass=<?= $ass_id; ?>&delusers=1">Delete Assessment Users</a>

                                                                   <li><a href="question_bank.php?ass_id=<?= $ass_id; ?>&tsid=<?= $testid; ?>" target="_blank">Edit Questions</a></li>

                                                                   <li><a href="?reset_all_time=<?= $ass_id; ?>&tsid=<?= $testid; ?>">Reset All Time</a> -->
                                                                <!-- 
                    ...
                    <div class="modal fade" id="EditTime" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form method="post" action="<?php echo $post_url; ?>&edittime=<?php echo$_GET['edittime']; ?>" name="edit_time" enctype="multipart/form-data">
                                    <div class="modal-body">
                                       <table class="table table-bordered table-condensed table-responsive">
                                                <thead >
                                                    <tr>
                                                ...
                                                <tbody>
                                                <?php  $user_id = $_GET['edittime'];
                                                 $prep_query = "SELECT  * 
                                                                FROM user_assessments 
                                                                WHERE ass_id = '$ass_id' 
                                                                AND ass_user_id = '$user_id'";
                                                        //print_r($prep_query);  
                                                        $users = mysqli_query($dbc, $prep_query);
                                                        while ( $row_users = mysqli_fetch_assoc($users)) {
                                                        echo '<tr>
                                                                <td>'.$row_users['ass_user_id'].'</td>
                                                                <td><input type="text" name="time" value="'.$row_users['time_length'].'" class="form-control" required width="100px">
                                                                </td>
                                                            </tr>';
                                                        }?>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                        <button type="submit" class="btn btn-primary" name="edit_user_time">Save Changes</button>
                                    </div>
                                </form>
                            </div>
                            <!-- /.modal-content -->
