...
$dir = basename(__DIR__);
 ...
 $prep_query = "SELECT DISTINCT batch FROM assessments_users"; 
...
echo   $prep_query = " SELECT us.id, fname, mname, lname, userid, password,  assessments_code, batch, login
                 FROM users us, assessments_users aus, assessments ass  WHERE userid = username 
                 AND ass.assessments_code = aus.assessment_code
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                ...
                                <span style="float:right;">
                                    <button class="btn btn-danger btn-xs " id="logoutall"><i class="fa fa-refresh" ></i> Logout All Users' Login  </button>
...
<!--                                <div class="dataTables_paginate paging_simple_numbers" id="dataTables-example_paginate">
                                ...
                                <div class="">
                                    <table class="table table-responsive table-bordered table-condensed ">
                                        ...
                                        <tbody>
                                            <tr class="odd gradeX" ng-repeat="user in data.users | filter:{$: criteria, batch: sescrit,  login: loginn} | limitTo:10">
                                                <td>{{$index +1}}</td>
                                                ...
                                                <td class="text-center">                                               
                                                    <button  class="btn btn-danger btn-outline btn-xs logout" ng-if='user.login == 1' data-value='{{user.userid}}'><i class="fa fa-stop"></i> End Exam</button>
                                                    <button  class="btn btn-info btn-outline btn-xs logout" ng-if='user.login == 1' data-value='{{user.userid}}'><i class="fa fa-pause"></i> Pause Exam</button>
                                                    <button  class="btn btn-danger btn-outline btn-xs logout" ng-if='user.login == 1' data-value='{{user.userid}}'><i class="fa fa-power-off"></i> Logout Exam</button>
                                                </td>
                                            </tr>                               
                                        </tbody>
                                    </table>
