...
<?php
    ...
    foreach ($rowactivets as $rowats) {
        ...
        if (($countuc !== 50) && ($be_default == 1)) {

           //RESET USER CHOICE
           $sql_delete = "DELETE FROM user_choice WHERE user_test_id = '$user_test_id' ";
           $sql_result = mysqli_query($dbc, $sql_delete);

           //RESET END EXAM
           $sql_update_end_exam = "UPDATE user_test SET finish = 0 WHERE user_id = '$uid' AND test_id = '$testidats' ";
           $sql_result = mysqli_query($dbc, $sql_update_end_exam);


           //RESET TIME
           $sql_update_time = "UPDATE user_assessments SET time_length = '00.00' WHERE ass_user_id = '$uid' ";
        //    print($sql_update_time); die();
           $sql_result = mysqli_query($dbc, $sql_update_time);
